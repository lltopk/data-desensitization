...
## 方案三 数据库加解密(不推荐)
数据库层面脱敏与密码存储一样，新增数据的时候, 存储脱敏后的值

与密码场景不同的是, 数据脱敏本质还是需要进行前端展示, 因此若加密了数据则必须再解密出明文

### 方案3.1 加密存储 查询解密
有双向加解密算法如AES，RSA，在随后的查询过程中在内存解密出明文
```sql
select * from user where add_time > '2024-08-01'
==> 放入内存
```

缺点显然易见, 当数据量大的时候，解密很耗时，且解密很耗内存
...
## 方案四 加密模糊查询(不推荐)
数据库加解密除了性能问题之外, 还无法进行模糊查询, 除非全量查询全量解密之后, 在内存中用明文模糊匹配, 这显然不现实

下面是两种支持加密模糊查询的方案

### 方案4.1 数据库建立起明文与密文的映射表
